...
export const getFbLiveVideos = async (
  ...
  let endpoint = `/${fansPage.id}/live_videos?access_token=${fansPage.access_token}&fields=id,title,description,creation_time,status,embed_html,video{id,title,description,created_time}&limit=${limit}`;
  if (cursor_after) {
    endpoint = `${endpoint}&after=${cursor_after}`;
  }
  await FB.api(
...
export const getPostInfo = async (
  ...
  const endpoint = `/${id}?access_token=${accessToken}&fields=id,message,created_time,from{id,name,picture{url}},status_type`;
  await FB.api(
    endpoint,
...
export const getIgMediaInfo = async (
  ...
  const endpoint = `/${media_fb_id}?access_token=${accessToken}&fields=id,timestamp,permalink,media_url,media_type,caption,comments_count,owner,username`;
  await FB.api(
    endpoint,
