  ...
  return (response: any) => {
    ...
    if (response?.error?.code === 190) {
      ...
      if (pathNames.length >= 4) {
        window.location.href = `/liveroom/activities/${pathNames[3]}/errors/accessTokenExpired`;
      }
...
export interface FANS_POST {
  ...
  error?: { code: number; fbtrace_id: string; message: string; type: string };
}
...
export interface IG_MEDIA {
  id: string;
  media_type: "BROADCAST" | "VIDEO" | "IMAGE";
  timestamp: string;
  caption?: string;
  permalink: string;
  media_url: string;
...
export interface IG_MEDIA_COMMENT {
  ...
  parent_id?: string;
  replies?: { data: IG_MEDIA_COMMENT[] };
}

export type BACKEND_COMMENT = {
  fb_id: string;
...
export type DISAPPEARED_IG_MEDIA = {
  id: string;
};
...
export const getGroupPosts = (
  ...
  let endpoint = `/${groupId}/feed?access_token=${accessToken}&fields=id,message,type,object_id,created_time&limit=${limit}`;
  if (cursor_after) {
    endpoint = `${endpoint}&after=${cursor_after}`;
  }
  if (url) {
...
export const getIgLiveMediaAndMedia = (
  ...
  let endpoint = `/${ig_business_account}/live_media?access_token=${accessToken}&fields=id,timestamp,caption&limit=${limit}`;
  if (cursor_after) {
    endpoint = `${endpoint}&after=${cursor_after}`;
  }
  FB.api(
...
export const getIgComments = async (
  ...
  let endpoint = `/${fansPost}/comments?access_token=${accessToken}&fields=id,text,timestamp,from{id,username}&limit=${limit}`;
  if (cursor_after) {
    endpoint = `${endpoint}&after=${cursor_after}`;
  }
  await FB.api(
...
export const getFansPage = async (
  ...
  const endpoint = `/${id}?access_token=${accessToken}&fields=id,name,access_token,picture{url},instagram_business_account{id,profile_picture_url,username}`;
  await FB.api(
    endpoint,
...
export const getFbLiveVideos = async (
  ...
  let endpoint = `/${fansPage.id}/live_videos?access_token=${fansPage.access_token}&fields=id,status,from&limit=${limit}`;
  if (cursor_after) {
    endpoint = `${endpoint}&after=${cursor_after}`;
  }
  await FB.api(
...
export const getGroupLiveVideos = async (
  ...
  let endpoint = `/${groupId}/live_videos?access_token=${accessToken}&fields=id,title,description,creation_time,stream_url,status,embed_html,video{id,title,description,created_time}&limit=${limit}`;
  if (cursor_after) {
    endpoint = `${endpoint}&after=${cursor_after}`;
  }
  await FB.api(
...
export const getPostInfo = async (
  ...
  const endpoint = `/${id}?access_token=${accessToken}&fields=attachments,id,message,created_time,from{id,name,picture{url}}`;
  await FB.api(
    endpoint,
...
export const updateLiveVideoDescription = async (
  pageId: string,
  accessToken: string,
  description: string,
  onSuccess: Function
) => {
  const liveVideos = await new Promise<any>((resolve, reject) => {
    const liveVideoEndpoint = `/${pageId}/live_videos?status=LIVE_NOW&access_token=${accessToken}&fields=id,description`;
    FB.api(
      liveVideoEndpoint,
      fbApiErrorHandler((response: FANS_POST) => {
        if (response) {
  ...
  const endpoint = `/${liveVideos?.data[0].id}?access_token=${accessToken}&fields=id,description`;
  await FB.api(
    endpoint,
    "POST",
    { description },
    fbApiErrorHandler((response: LIVE_VIDEO) => {
      if (response) {
        onSuccess && onSuccess(response.description);
      }
    }, endpoint)
  );
};
export const updateVideoDescription = async (
  ...
  const endpoint = `/${id}?access_token=${accessToken}&fields=id,description`;
  await FB.api(
    endpoint,
    "POST",
    { description },
    fbApiErrorHandler((response: VIDEO) => {
      if (response) {
        onSuccess && onSuccess(response.description);
      }
    }, endpoint)
  ...
  return new Promise((resolve, reject) => {
    api
      ...
      .then(() => {
        ...
        eventNode.addEventListener("error", (e) => {
          reject(e);
        });
      });
  });
...
export const getIgMediaInfo = async (
  accessToken: string,
  media_fb_id: string,
  onSuccess: Function
) => {
  const endpoint = `/${media_fb_id}?access_token=${accessToken}&fields=id,timestamp,permalink,media_url,media_type,caption,comments_count,owner`;
  await FB.api(
    endpoint,
    fbApiErrorHandler((response: IG_MEDIA) => {
      onSuccess(response);
    }, endpoint)
  );
};

export const fetchIgHistoricalComments = async (
  accessToken: string,
  ig_media_fb_id: string,
  onSuccess?: Function,
  cursor_after?: string | null
) => {
  let endpoint = `/${ig_media_fb_id}/comments?access_token=${accessToken}&fields=id,text,timestamp,from{id,username},replies.limit(10){id,text,timestamp,from{id,username},parent_id}&limit=500&order=chronological`;
  if (cursor_after) {
    endpoint = `${endpoint}&after=${cursor_after}`;
  }
  await FB.api(
...
export const fetchIgUserAvatar = async (
  accessToken: string,
  ig_user_id: string,
  onSuccess?: Function
) => {
  const endpoint = `/${ig_user_id}?access_token=${accessToken}&fields=profile_pic`;
  await FB.api(
    endpoint,
    fbApiErrorHandler((response: { success: boolean }) => {
      if (response.success) {
...
export const getIgLiveVideoComment = () => {
...
export const sendIGPostComment = async (
  accessToken: string,
  postId: string,
  message: string,
  onSuccess: Function
) => {
  const endpoint = `/${postId}/comments?access_token=${accessToken}&fields=id,text,timestamp,from{id,username},parent_id`;

  await FB.api(
    endpoint,
    "POST",
    { message: message },
    fbApiErrorHandler((response: IG_MEDIA_COMMENT) => {
      if (response) {
        onSuccess && onSuccess(response);
      }
    }, endpoint)
...
export const editFBComment = async (
  accessToken: string,
  commentId: string,
  message: string,
  onSuccess: Function
...
export const getLiveVideoDetail = async (
  accessToken: string,
  videoId: string,
  onSuccess: Function
) => {
  const endpoint = `/${videoId}?access_token=${accessToken}&fields=live_status,created_time,id,title,description,status,embed_html`;
...
export const getLiveVideoViewer = async (
  accessToken: string,
  videoId: string,
  onSuccess: Function
) => {
  const endpoint = `/${videoId}?access_token=${accessToken}&fields=live_views`;
...
export const finishedLiveVideo = async (
  accessToken: string,
  videoId: string,
  onSuccess: Function
) => {
  const endpoint = `/${videoId}?access_token=${accessToken}&end_live_video=true`;

  await FB.api(
    endpoint,
    "POST",
    fbApiErrorHandler((response: { id: string }) => {
      if (response) {
        onSuccess && onSuccess(response);
      }
    }, endpoint)
