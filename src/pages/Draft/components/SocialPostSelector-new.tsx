...
function SocialPostSelector(props: PROPS) {
    ...
    if (curFansPage && curPlatform) {
      ...
      if (curPage && curLiveVideos.length && curLiveVideo === "") {
        ...
        if (liveVideo) {
          ...
          onPostChange(curPostId, { ...getCurSocia(), ...liveVideo });
  ...
  useEffect(() => {
    if (curGroupPosts && groupLiveVideos) {
      ...
      if (livePost.length) {
        ...
        onPostChange(livePost[0].id, { ...getCurSocia(), ...livePost[0] });
      }
    }
  ...
  const getCurSocia = () => {
    if (curPlatform) {
      switch (channel) {
        case "facebook.page":
          return curPlatform[channel].find((item) => item.id === curFansPage);
        case "facebook.group":
          return curPlatform[channel].find((item) => item.id === curGroup);
        case "instagram":
          return curPlatform[channel].find((item) => item.id === curIgFansPage);
        default:
          return {};
      }
    }
              ...
              <ListItemButton
                ...
                onClick={(event) => {
                  ...
                  if (curPage) {
                    ...
                    onPostChange(curPostId, { ...getCurSocia(), ...item });
                    setCurLiveVideo(curPostId);
                  }
                }}
              >
          ...
          <List component="nav" aria-label="group page live post list">
            {groupLivePosts.map((item, index) => (
              <ListItemButton
                ...
                onClick={(event) => {
                  onPostChange(curGroupLivePost, { ...getCurSocia(), ...item });
