...
export const getFbVideos = async (
  ...
  let endpoint = `/${fansPage.id}/videos?access_token=${fansPage.access_token}&fields=post_id,live_status,created_time,id,video_id,title,description,live_views,embed_html&limit=${limit}`;
  if (cursor_after) {
    endpoint = `${endpoint}&after=${cursor_after}`;
  }
  await FB.api(
...
//   onSuccess: Function
// ) => {
//   const endpoint = `/${videoId}/comment?access_token=${accessToken}&fields=embed_html`;
