...
function Activity() {
  ...
  const getLivehostUrl = () => {
    navigator.clipboard.writeText(
      `https://${
        window.location.host
      }/livehost/activities/${activityId}/posts/${postId}?api=${
    ...
    <>
      {activity && (
        ...
        >
          ...
          >
            {activity &&
              (activity.dispatch?.platform.includes("facebook") ||
                activity.dispatch?.platform === "instagram") && (
                <Tooltip title="分享直播主模式" placement="bottom" arrow>
                  <IconButton sx={{ mr: 1 }} onClick={() => getLivehostUrl()}>
                    <LinkOutlinedIcon />
                  </IconButton>
                </Tooltip>
              )}
